...
func refresh_from_stats(stats) -> void:
	...
	var xp_next: int = stats.get_xp_for_next_level() if stats.get_xp_for_next_level() > 0 else 1
	var rows := [
		{"label": "Level", "value": "%d" % stats.level()},
		{"label": "XP", "value": "%d / %d" % [stats.get_xp(), xp_next]},
		{"label": "Total $", "value": "$%.2f" % stats.total_spent()},
		{"label": "Checkouts", "value": "%d" % stats.checkout_count()},
		{"label": "Items Bought", "value": "%d" % stats.items_bought()},
	]
