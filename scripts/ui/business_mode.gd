...
func _make_staff_row(name: String, role: String, status: String, perf: int, morale: String = "OK") -> HBoxContainer:
	var row := HBoxContainer.new()
	var name_lbl := Label.new()
	name_lbl.text = "  " + name.substr(0, mini(name.length(), 16))
	name_lbl.add_theme_color_override("font_color", Color(0.82, 0.82, 0.75))
	name_lbl.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	row.add_child(name_lbl)
	var role_lbl := Label.new()
	role_lbl.text = role.substr(0, mini(role.length(), 18))
	role_lbl.add_theme_color_override("font_color", Color(0.60, 0.65, 0.70))
	row.add_child(role_lbl)
	var status_lbl := Label.new()
	status_lbl.text = status.substr(0, mini(status.length(), 10))
...
func _draw_analytics(vbox: VBoxContainer) -> void:
	...
	var metrics := [
		{"label": "Busiest Hour", "value": "11AM-1PM", "note": "Lunch rush"},
	...
	# Simple bar chart using labels - compact display
	...
	for i in range(hours.size()):
		...
		hour_lbl.text = hours[i]
		hour_lbl.add_theme_color_override("font_color", Color(0.60, 0.60, 0.65))
		hour_lbl.size_flags_horizontal = Control.SIZE_SHRINK_BEGIN
		hour_lbl.custom_minimum_size = Vector2(32, 0)
		row.add_child(hour_lbl)
		var bar_lbl := Label.new()
		var bar_len := int(traffic[i] / max_t * 20.0)
		bar_lbl.text = "|"
		for j in range(bar_len):
			bar_lbl.text += "="
		bar_lbl.text += " %d%%" % traffic[i]
