...
func init_all() -> void:
	...
	game_clock.hour_changed.connect(m._logic._on_hour_changed)
	game_clock.day_changed.connect(m._logic._on_day_changed)
	game_clock.shift_report.connect(m._logic._on_shift_report)
	...
	brand_portal.closed.connect(m._logic._on_brand_portal_closed)
	...
	maintenance_system.issue_created.connect(m._logic._on_issue_created)
	maintenance_system.issue_resolved.connect(m._logic._on_issue_resolved)
	...
	player_stats.achievement_unlocked.connect(m._logic._on_achievement_unlocked)
	player_stats.level_up.connect(m._logic._on_player_level_up)
	player_stats.staff_rank_up.connect(m._logic._on_staff_rank_up)
	...
	if save_sys.load_game(m):
		...
		tutorial_overlay.dismissed.connect(m._logic._on_tutorial_dismissed)
	...
	daily_bonus.streak_reward.connect(m._logic._on_streak_reward)
	...
	quest_system.quest_completed.connect(m._logic._on_quest_completed)
	quest_system.all_daily_complete.connect(m._logic._on_all_quests_complete)
	...
	settings_panel.setting_changed.connect(m._logic._on_setting_changed)
	...
	pause_menu.paused.connect(m._logic._on_game_paused)
	pause_menu.resumed.connect(m._logic._on_game_resumed)
	...
	# ── Hover Panel ────────────────────────────────────────────────────────────
	# Floating panel that shows when the cursor sits on an NPC or robot.
	# Controllers locate it via the "hover_panel" group.
	var hover_panel = preload("res://scripts/ui/hover_panel.gd").new()
	...
	section_browse.item_added.connect(m._logic._on_item_added_to_cart)
	section_browse.closed.connect(m._logic._on_browse_closed)
	...
	food_stall_browse.item_added.connect(m._logic._on_item_added_to_cart)
	...
	if m.DEV_MODE:
		...
		dev_tools.dev_commandIssued.connect(m._logic._on_dev_command)
