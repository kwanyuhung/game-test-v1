...
# Accessed via: const StoreData = preload("res://scripts/world/store_data.gd")
...
static func _static_init() -> void:
	SECTIONS = [
		# Floor 0 — Ground Floor Fresh Market (8 sections: 2 rows × 4 cols)
		...
		SectionDef.new("beverages", "BEVERAGES",      60,  3, 20, 14, SectionStyle.FRIDGE,   Color(0.45, 0.72, 0.95),  "V",  0),
		SectionDef.new("meat",      "MEAT & DELI",    20, 19, 20, 14, SectionStyle.DELI,     Color(0.95, 0.72, 0.68),  "M",  0),
		SectionDef.new("seafood",   "SEAFOOD",        42, 19, 16, 14, SectionStyle.FRIDGE,   Color(0.40, 0.72, 0.88),  "F",  0),
		SectionDef.new("frozen",    "FROZEN",         60, 19, 18, 14, SectionStyle.FREEZER,  Color(0.78, 0.92, 1.00),  "Z",  0),
		SectionDef.new("pantry",    "PANTRY & SNACKS", 2, 19, 16, 14, SectionStyle.SHELF,    Color(0.88, 0.78, 0.55),  "N",  0),
		...
		# Floor 4 — Outdoor
		# Floor 5 — Frozen (moved to Floor 0, see above)
	...
	CATALOG = [
		...
		# ═══════════════════ DRINKS ═══════════════════
		MarketProduct.new("water_sparkling","Sparkling Water 1L", 1.20, Color(0.80, 0.92, 1.00), 5, "beverages", "Water",       "Crisp carbonated water. Zero sugar, all bubbles."),
		MarketProduct.new("water_still",  "Spring Water 1L",    0.80, Color(0.78, 0.90, 1.00), 5, "beverages", "Water",       "Pure mountain spring water. Clean and refreshing."),
		MarketProduct.new("juice_orange", "Orange Juice 1L",     3.50, Color(0.98, 0.65, 0.10), 5, "beverages", "Juice",       "Fresh-squeezed style OJ. Morning vitamin boost."),
		MarketProduct.new("juice_apple",  "Apple Juice 1L",      3.00, Color(0.90, 0.78, 0.42), 5, "beverages", "Juice",       "Sweet and crisp. Kids' favourite."),
		MarketProduct.new("juice_grape",  "Grape Juice 1L",      3.20, Color(0.55, 0.18, 0.72), 5, "beverages", "Juice",       "Rich purple grape. Tastes like Welch's."),
		MarketProduct.new("juice_cran",   "Cranberry Juice 1L",  3.80, Color(0.72, 0.18, 0.28), 5, "beverages", "Juice",       "Tart and tangy. Great mixed with other juices."),
		MarketProduct.new("soda_cola",    "Cola 2L",             2.20, Color(0.38, 0.22, 0.18), 5, "beverages", "Soda",        "Classic cola. Ice cold is the only way."),
		MarketProduct.new("soda_sprite",   "Lemon-Lime Soda 2L", 2.20, Color(0.88, 0.98, 0.48), 5, "beverages", "Soda",        "Clear and citrusy. Ultra refreshing."),
		MarketProduct.new("soda_orange",  "Orange Soda 2L",      2.20, Color(0.98, 0.58, 0.10), 5, "beverages", "Soda",        "Bright and sweet orange fizz."),
		MarketProduct.new("soda_rootbeer","Root Beer 2L",         2.20, Color(0.72, 0.52, 0.32), 5, "beverages", "Soda",        "Creamy and sassafras-y.Pairs with vanilla ice cream."),
		MarketProduct.new("soda_grape",   "Grape Soda 2L",       2.20, Color(0.55, 0.18, 0.72), 5, "beverages", "Soda",        "Bold purple fizz. Fun and fruity."),
		MarketProduct.new("energy_redbull","Red Bull 250ml",     3.50, Color(0.95, 0.78, 0.18), 5, "beverages", "Energy",      "Wings in a can. Gives you that edge."),
		MarketProduct.new("energy_monster","Monster Energy 500ml",3.80, Color(0.38, 0.62, 0.28), 5, "beverages", "Energy",      "Big bold energy. Not for the caffeine-sensitive."),
		MarketProduct.new("coffee_blend", "House Blend Coffee",  8.50, Color(0.38, 0.25, 0.18), 5, "beverages", "Coffee",      "Pre-ground medium roast. Consistent and smooth."),
		MarketProduct.new("coffee_espresso","Espresso Beans 250g",9.00, Color(0.35, 0.22, 0.15), 5, "beverages", "Coffee",      "Italian roast espresso beans. Dark and bold."),
		MarketProduct.new("tea_earlgrey", "Earl Grey Tea 20pk",  3.50, Color(0.92, 0.88, 0.72), 5, "beverages", "Tea",         "Bergamot-infused black tea. Elegant and aromatic."),
		MarketProduct.new("tea_green",    "Green Tea 20pk",      3.20, Color(0.55, 0.78, 0.52), 5, "beverages", "Tea",         "Gentle Japanese green tea. Light and grassy."),
		MarketProduct.new("tea_chamomile","Chamomile Tea 20pk",  3.80, Color(0.95, 0.90, 0.60), 5, "beverages", "Tea",         "Soothing herbal tea. Perfect before bed."),
		MarketProduct.new("lemonade",     "Lemonade 1L",         2.50, Color(0.98, 0.95, 0.50), 5, "beverages", "Juice",       "Homemade-style lemonade. Sweet and tart."),
		MarketProduct.new("iced_tea",     "Iced Tea 1L",         2.00, Color(0.88, 0.72, 0.48), 5, "beverages", "Tea",         "Bottled iced tea. Lightly sweetened."),
		MarketProduct.new("kombucha",      "Kombucha 500ml",      4.50, Color(0.38, 0.72, 0.62), 5, "beverages", "Juice",       "Fermented tea. Probiotic and slightly effervescent."),

		# ═══════════════════ SNACKS ═══════════════════
		MarketProduct.new("chips_salted", "Salted Chips 200g",   3.20, Color(0.95, 0.82, 0.42), 3, "pantry", "Chips",       "Classic salted potato chips. Crispy and addictive."),
		MarketProduct.new("chips_bbq",    "BBQ Chips 200g",     3.20, Color(0.72, 0.32, 0.18), 3, "pantry", "Chips",       "Smoky BBQ seasoned chips. Tangy and savory."),
		MarketProduct.new("chips_sour",   "Sour Cream & Onion",  3.20, Color(0.90, 0.95, 0.78), 3, "pantry", "Chips",       "Cool sour cream with chive. Crowd pleaser."),
		MarketProduct.new("chips_choco",  "Chocolate Chips 200g",3.50, Color(0.42, 0.28, 0.18), 3, "pantry", "Chips",       "Sweet chocolate-dusted chips. Dessert chip."),
		MarketProduct.new("tortilla_chips","Tortilla Chips 300g", 3.00, Color(0.92, 0.82, 0.52), 3, "pantry", "Chips",       "Sturdy corn chips. Made for salsa and guac."),
		MarketProduct.new("pretzels",     "Pretzel Twists 200g", 2.80, Color(0.78, 0.58, 0.32), 3, "pantry", "Chips",       "Salted soft pretzels. Beer best friend."),
		MarketProduct.new("popcorn_butter","Butter Popcorn 150g",2.50, Color(0.98, 0.90, 0.50), 3, "pantry", "Chips",       "Movie theater style butter popcorn in a bag."),
		MarketProduct.new("cookies_choc", "Chocolate Cookies 200g",3.20, Color(0.42, 0.28, 0.18), 3, "pantry", "Cookies",     "Chewy chocolate chip cookies. Straight from the oven taste."),
		MarketProduct.new("cookies_oat",  "Oatmeal Raisin 200g", 3.00, Color(0.72, 0.55, 0.32), 3, "pantry", "Cookies",     "Chewy oatmeal cookies with raisins and cinnamon."),
		MarketProduct.new("cookies_sugar","Sugar Cookies 200g",   2.80, Color(0.95, 0.88, 0.72), 3, "pantry", "Cookies",     "Sweet and crumbly. Great with milk."),
		MarketProduct.new("cookies_thin", "Wafer Cookies 300g",   3.50, Color(0.95, 0.92, 0.80), 3, "pantry", "Cookies",     "Layered vanilla wafers. Light and crispy."),
		MarketProduct.new("candy_gummy",  "Gummy Bears 150g",     2.50, Color(0.92, 0.28, 0.28), 0, "pantry", "Candy",       "Chewy fruity bears. One is never enough."),
		MarketProduct.new("candy_sour",   "Sour Worms 150g",     2.80, Color(0.98, 0.88, 0.28), 0, "pantry", "Candy",       "Tart sugar-coated sour gummy worms."),
		MarketProduct.new("candy_choc",   "Milk Chocolate Bar",   2.50, Color(0.55, 0.38, 0.22), 3, "pantry", "Chocolate",   "Smooth milk chocolate. The classic treat."),
		MarketProduct.new("candy_dark",   "Dark Chocolate 70%",   3.50, Color(0.28, 0.18, 0.12), 3, "pantry", "Chocolate",   "Rich and bittersweet. Cocoa-forward."),
		MarketProduct.new("chocolate盒",  "Chocolate Box 12pk",   8.00, Color(0.55, 0.35, 0.20), 3, "pantry", "Chocolate",   "Assorted chocolate truffles. Gift-worthy."),
		MarketProduct.new("nuts_mixed",   "Mixed Nuts 300g",     6.50, Color(0.75, 0.58, 0.35), 0, "pantry", "Nuts",        "Roasted and salted. Healthy snacking."),
		MarketProduct.new("nuts_almonds", "Roasted Almonds 200g",5.50, Color(0.78, 0.62, 0.42), 0, "pantry", "Nuts",        "Crunchy roasted almonds. High in protein."),
		MarketProduct.new("nuts_peanuts", "Roasted Peanuts 300g",3.50, Color(0.82, 0.55, 0.32), 0, "pantry", "Nuts",        "Salted peanuts. The ballgame classic."),
		MarketProduct.new("nuts_pistachio","Pistachios 200g",    7.50, Color(0.62, 0.88, 0.52), 0, "pantry", "Nuts",        "In-shell roasted pistachios. Addictive and fun."),
		MarketProduct.new("granola_bar",  "Granola Bars 6pk",    3.80, Color(0.85, 0.68, 0.42), 3, "pantry", "Bars",        "Oat and honey cereal bars. On-the-go energy."),
		MarketProduct.new("fruit_snack",  "Dried Mango 150g",    4.50, Color(0.98, 0.72, 0.28), 0, "pantry", "Nuts",        "Sweet dried mango slices. Tropical treat."),
		MarketProduct.new("hummus",       "Hummus 200g",         3.50, Color(0.85, 0.78, 0.52), 4, "pantry", "Dips",        "Creamy chickpea dip. Great with pita."),
		MarketProduct.new("salsa",        "Salsa Medium 400g",  2.80, Color(0.92, 0.22, 0.22), 4, "pantry", "Dips",        "Tomato salsa with jalape\u00f1o. Medium heat."),
		MarketProduct.new("guac",         "Guacamole 200g",       4.00, Color(0.55, 0.78, 0.42), 4, "pantry", "Dips",        "Fresh avocado dip. Eat within 24 hours."),
		...
		MarketProduct.new("salmon_fillet","Fresh Salmon 300g",  10.50, Color(0.92, 0.58, 0.48), 3, "seafood", "Fish",        "Fresh Atlantic salmon fillet. Omega-3 rich."),
		MarketProduct.new("tuna_steak",   "Tuna Steak 250g",     11.00, Color(0.55, 0.42, 0.48), 3, "seafood", "Fish",        "Sushi-grade tuna. Flash-frozen for freshness."),
		MarketProduct.new("shrimp",       "Tiger Shrimp 300g",   9.50, Color(0.98, 0.72, 0.60), 0, "seafood", "Shellfish",   "Peeled and deveined. Quick weeknight protein."),
		MarketProduct.new("cod_fillet",   "Cod Fillet 400g",     7.50, Color(0.92, 0.90, 0.85), 3, "seafood", "Fish",        "White flaky fish. Classic fish and chips fish."),
