# fashion_common_handler.gd
# ─────────────────────────────────────────────────────────────────────────────
# Handler for COMMON zones on Floor 2 (Fashion)
# General walking/floor areas with fashion floor styling
# ─────────────────────────────────────────────────────────────────────────────
class_name FashionCommonHandler

const CELL_SIZE := 16

static func build_fashion_common(parent: Node, zone: Dictionary, floor_nodes: Array) -> void:
	var cx: int = zone.x * CELL_SIZE
	var cy: int = zone.y * CELL_SIZE
	var cw: int = zone.w * CELL_SIZE
	var ch: int = zone.h * CELL_SIZE
	
	# Base floor with elegant tile pattern
	var bg := ColorRect.new()
	bg.position = Vector2(cx, cy)
	bg.size = Vector2(cw, ch)
	bg.color = Color(0.22, 0.20, 0.22)
	parent.add_child(bg)
	floor_nodes.append(bg)
	
	# Subtle floor pattern (tile lines)
	var tile_color := Color(0.20, 0.18, 0.20)
	...
	edge_light.color = Color(0.28, 0.25, 0.28, 0.5)
